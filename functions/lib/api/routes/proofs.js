...
const MAX_PROOF_BYTES = limits_1.MAX_BYTES.PROOF;
...
const MAX_PATH_LEN = limits_1.MAX_LEN.PATH;
exports.proofsRouter = (0, express_1.Router)();
// ─── POST /proofs/upload ──────────────────────────────────────────────────────
/**
 ...
 */
exports.proofsRouter.post("/upload", auth_1.requireAuth, (0, auth_1.requireAnyRole)("driver"), async (req, res) => {
    let parsed;
    try {
 ...
 */
exports.proofsRouter.get("/url", auth_1.requireAuth, async (req, res) => {
    ...
    try {
        const url = await (0, storage_1.getDownloadURL)((0, storage_1.getStorage)().bucket().file(path));
        res.json({ url });
    }
    catch (err) {
 ...
 */
function canReadProof(req, groomUid) {
    const claims = req.caller.claims;
 ...
 */
function buildProofPath(groomUid, guestId, contentType) {
