 ...
 */
exports.settingsRouter.get("/", auth_1.requireAuth, async (_req, res) => {
    try {
        const snap = await (0, database_1.getDatabase)().ref("adminSettings").get();
        res.json(snap.val() ?? {});
    }
    catch (err) {
 ...
 */
exports.settingsRouter.patch("/", auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        await (0, database_1.getDatabase)().ref("adminSettings").update(validation.patch);
        const snap = await (0, database_1.getDatabase)().ref("adminSettings").get();
        res.json(snap.val() ?? {});
    }
    catch (err) {
