...
const MAX_GUEST_RANK_LEN = limits_1.MAX_LEN.GUEST_RANK;
const MAX_GUEST_NOTE_LEN = limits_1.MAX_LEN.NOTE;
const MAX_BRIDE_NAME_LEN = limits_1.MAX_LEN.NAME;
...
const MAX_INVITE_MEDIA_BYTES = limits_1.MAX_BYTES.INVITE_MEDIA;
const MAX_PHOTOG_BYTES = limits_1.MAX_BYTES.PHOTOGRAPHER;
...
exports.digitalRouter = (0, express_1.Router)();
// ═══════════════════════════════════════════════════════════════════════════════
 ...
 */
exports.digitalRouter.get("/:uid/guests", auth_1.requireAuth, async (req, res) => {
 ...
 */
exports.digitalRouter.post("/:uid/guests", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
    ...
    try {
        ...
        const dup = existing.docs.some((d) => ilNational((d.data().phone ?? "").toString()) === newNat);
        if (dup) {
 ...
 */
exports.digitalRouter.patch("/:uid/guests/:id", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
...
exports.digitalRouter.delete("/:uid/guests/:id", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
...
exports.digitalRouter.get("/:uid/wishes", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
...
exports.digitalRouter.patch("/:uid/wishes/:wishId", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
...
exports.digitalRouter.delete("/:uid/wishes/:wishId", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.get(["/:uid/media", "/:uid/designs/:designId"], auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
    try {
        ...
        const design = projectMediaDoc(dSnap.data());
 ...
 */
exports.digitalRouter.patch(["/:uid/media/settings", "/:uid/designs/:designId"], auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
    ...
    try {
        ...
        for (const [k, v] of Object.entries(sanitized.value)) {
            if (k === "photographerPublished" || k === "guestRanks")
                parentPatch[k] = v;
            else
                designPatch[k] = v;
        }
 ...
 */
exports.digitalRouter.get("/:uid/settings", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
...
exports.digitalRouter.patch("/:uid/settings", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
    ...
    const patch = {};
    for (const k of ["photographerPublished", "guestRanks"]) {
        if (k in sanitized.value)
            patch[k] = sanitized.value[k];
    }
 ...
 */
exports.digitalRouter.post(["/:uid/media/upload", "/:uid/designs/:designId/media/upload"], auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.post(["/:uid/media/delete-item", "/:uid/designs/:designId/media/delete-item"], auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.delete("/:uid/media", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
...
exports.digitalRouter.get("/:uid/designs", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.post("/:uid/designs", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.delete("/:uid/designs/:designId", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
    try {
        ...
        const currentDefault = pSnap.exists ? pSnap.data()?.defaultDesignId : null;
        ...
        await batch.commit();
        await parentDoc(uid).set({ defaultDesignId: newDefault, designCount: all.size - 1 }, { merge: true });
 ...
 */
exports.digitalRouter.get("/:uid/photographer", async (req, res) => {
 ...
 */
exports.digitalRouter.post("/:uid/photographer/upload", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.patch("/:uid/photographer/:fileId", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.delete("/:uid/photographer/:fileId", auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.get("/design-list", auth_1.requireAuth, auth_1.requireAdmin, async (_req, res) => {
    try {
        const snap = await (0, firestore_1.getFirestore)().collectionGroup(COLL_DESIGNS).get();
        const rows = snap.docs.map((d) => {
            const data = d.data() || {};
 ...
 */
exports.digitalRouter.post(["/:uid/design/submit", "/:uid/designs/:designId/design/submit"], auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
    try {
        ...
        const result = await fs().runTransaction(async (tx) => {
            ...
            return { ok: true, version, submittedAt: now };
        });
        if (!result.ok) {
 ...
 */
exports.digitalRouter.post(["/:uid/design/cancel", "/:uid/designs/:designId/design/cancel"], auth_1.requireAuth, async (req, res) => {
    if (!canActOnUid(req, req.params.uid)) {
        res.status(403).json({ error: "forbidden" });
        return;
    }
 ...
 */
exports.digitalRouter.post(["/:uid/design/approve", "/:uid/designs/:designId/design/approve"], auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
    try {
        const docRef = designDoc(req.params.uid, await resolveDesignId(req));
        const result = await fs().runTransaction(async (tx) => {
            const snap = await tx.get(docRef);
 ...
 */
exports.digitalRouter.post(["/:uid/design/reject", "/:uid/designs/:designId/design/reject"], auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
 ...
 */
exports.digitalRouter.post("/:uid/designs/:designId/design/set-status", auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
    const status = (req.body?.status ?? "").toString();
 ...
 */
exports.digitalRouter.get(["/:uid/public", "/:uid/designs/:designId/public"], async (req, res) => {
    try {
        const uid = req.params.uid;
...
function parentDoc(uid) {
    return fs().doc(`${COLL_ROOT}/${uid}`);
}
function mediaDoc(uid) {
    return parentDoc(uid);
}
function designsCol(uid) {
    return fs().collection(`${COLL_ROOT}/${uid}/${COLL_DESIGNS}`);
}
function designDoc(uid, designId) {
 ...
 */
async function ensureMigrated(uid) {
    ...
    return fs().runTransaction(async (tx) => {
        ...
        const legacy = data ? projectMediaDoc(data) : {};
        const designPayload = {};
        for (const [k, v] of Object.entries(legacy || {})) {
            if (!PARENT_ONLY_KEYS.has(k))
                designPayload[k] = v;
        }
        if (!designPayload.title)
            designPayload.title = { ar: "التصميم الأساسي", he: "עיצוב ראשי" };
        designPayload.order = 0;
        if (designPayload.createdAt == null)
            designPayload.createdAt = Date.now();
        if (designPayload.designStatus == null)
            designPayload.designStatus = "draft";
        if (designPayload.designVersion == null)
            designPayload.designVersion = 1;
...
async function resolveDefaultDesignId(uid) {
 ...
 */
async function deleteStorageObjectSilently(path) {
    try {
        await (0, storage_1.getStorage)().bucket().file(path).delete();
    }
    catch (err) {
...
async function deleteStorageFolder(prefix) {
    try {
        const [files] = await (0, storage_1.getStorage)().bucket().getFiles({ prefix });
 ...
 */
function canActOnUid(req, uid) {
    const claims = req.caller.claims;
    if (claims.role === "admin")
        return true;
    if (req.caller.uid === uid)
        return true;
    return false;
}
/**
 ...
 */
async function authenticatePhotographerRead(req) {
    const docSnap = await mediaDoc(req.params.uid).get();
    const published = docSnap.exists && docSnap.data()?.photographerPublished === true;
    if (published)
        return { ok: true };
...
function coerceRanks(data) {
    ...
    for (const r of raw) {
        ...
        if (!cleaned.includes(v))
            cleaned.push(v);
        if (cleaned.length >= MAX_RANK_ITEMS)
            break;
    }
...
function sanitizeDigitalGuestCreate(body) {
    ...
    if (data.designId !== undefined) {
        ...
        if (v)
            out.designId = v.slice(0, 200);
    }
    return { ok: true, value: out };
}
function sanitizeDigitalGuestPatch(body) {
    if (!body || typeof body !== "object" || Array.isArray(body)) {
        return { ok: false, error: "invalid_body" };
    }
    const data = body;
...
function sanitizeMediaSettings(body) {
    if (!body || typeof body !== "object" || Array.isArray(body)) {
        return { ok: false, error: "invalid_body" };
    }
    const data = body;
    ...
    if (data.weddingDate !== undefined) {
        ...
        else if (typeof data.weddingDate === "number" &&
    ...
    for (const [key, max] of localizedScalars) {
        if (data[key] !== undefined) {
            out[key] = clampLocalized(data[key], max);
        }
    }
    // ── Story timeline ───────────────────────────────────────────────────────
    if (data.storyTimeline !== undefined) {
        ...
        for (const raw of data.storyTimeline) {
            ...
            if (item.when || item.title || item.body)
                cleaned.push(item);
            if (cleaned.length >= MAX_STORY_ITEMS)
                break;
        }
    ...
    if (data.details !== undefined) {
        ...
        for (const raw of data.details) {
            ...
            if (item.meta || item.title || item.body)
                cleaned.push(item);
            if (cleaned.length >= MAX_DETAIL_ITEMS)
                break;
        }
        out.details = cleaned;
    }
    ...
    if (data.hotels !== undefined) {
        ...
        for (const raw of data.hotels) {
            ...
            if (item.name)
                cleaned.push(item);
            if (cleaned.length >= MAX_HOTEL_ITEMS)
                break;
        }
        out.hotels = cleaned;
    }
    ...
    if (data.wishes !== undefined) {
        ...
        for (const raw of data.wishes) {
            ...
            if (item.what)
                cleaned.push(item);
            if (cleaned.length >= MAX_WISH_ITEMS)
                break;
        }
        out.wishes = cleaned;
    }
    ...
    if (data.mealOptions !== undefined) {
        ...
        for (const r of data.mealOptions) {
            ...
            if (!v)
                continue;
            cleaned.push(v);
            if (cleaned.length >= MAX_MEAL_OPTIONS)
                break;
        }
    ...
    if (data.mediaCaptions !== undefined) {
        ...
        const cleaned = {};
        let count = 0;
        for (const [k, val] of Object.entries(data.mediaCaptions)) {
            if (count >= MAX_CAPTION_ENTRIES)
                break;
    ...
    for (const key of boolKeys) {
        if (data[key] !== undefined) {
            if (typeof data[key] !== "boolean") {
                return { ok: false, error: "invalid_toggle", field: key };
            }
            out[key] = data[key];
        }
    }
 ...
 */
function clampLocalized(v, max) {
    if (typeof v === "string")
        return v.trim().slice(0, max);
    if (v && typeof v === "object" && !Array.isArray(v)) {
        const o = v;
        const out = {};
        if (typeof o.ar === "string" && o.ar.trim())
            out.ar = o.ar.trim().slice(0, max);
        if (typeof o.he === "string" && o.he.trim())
            out.he = o.he.trim().slice(0, max);
 ...
 */
function migrateLegacyBackground(data, existing) {
...
function legacyBackgroundKind(type) {
    if (type === "video")
        return "video";
    if (type === "gif")
        return "gif";
...
function hasAllowedPrefix(contentType, prefixes) {
    return prefixes.some((p) => contentType.startsWith(p));
}
function pickExtensionFromFilename(filename, fallback) {
    const dot = filename.lastIndexOf(".");
    if (dot < 0)
        return fallback;
    const ext = filename.slice(dot + 1).toLowerCase();
    if (/^[a-z0-9]+$/.test(ext) && ext.length <= 8)
        return ext;
    return fallback;
}
