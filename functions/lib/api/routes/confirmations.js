...
exports.confirmationsRouter = void 0;
...
const SUBMIT_MAX_PER_HOUR = rateLimits_1.RATE.CONFIRM_PER_IP.limit;
const ATTACH_MAX_PER_HOUR = rateLimits_1.RATE.ATTACH_LOC_PER_ADMIN.limit;
...
exports.confirmationsRouter = (0, express_1.Router)();
// ─── GET /confirmations ───────────────────────────────────────────────────────
/**
 ...
 */
exports.confirmationsRouter.get("/", auth_1.requireAuth, auth_1.requireAdmin, async (_req, res) => {
    try {
        const snap = await (0, database_1.getDatabase)().ref("confirmations").get();
        const out = [];
        snap.forEach((c) => {
            const id = c.key;
            if (!id)
 ...
 */
exports.confirmationsRouter.patch("/:id", auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        const ref = (0, database_1.getDatabase)().ref(`confirmations/${id}`);
 ...
 */
exports.confirmationsRouter.post("/", (0, rateLimit_1.ipRateLimit)("confirm", SUBMIT_MAX_PER_HOUR, time_1.HOUR_MS), async (req, res) => {
 ...
 */
exports.confirmationsRouter.post("/attach-location", auth_1.requireAuth, auth_1.requireAdmin, (0, rateLimit_1.uidRateLimit)("attach", ATTACH_MAX_PER_HOUR, time_1.HOUR_MS), async (req, res) => {
 ...
 */
function buildConfirmationRecord(groomUid, s) {
    const now = Date.now();
    const record = {
        groomUid,
 ...
 */
async function tryAutoAttach(groomUid, s) {
    try {
        const target = (0, helpers_1.normalisePhoneForMatching)(s.submittedPhone);
        if (!target)
            return null;
        const db = (0, database_1.getDatabase)();
        ...
        const now = Date.now();
        const guestPatch = { confirmedAt: now };
        if (s.companions !== null)
            guestPatch.companions = s.companions;
 ...
 */
function sanitizeConfirmationPatch(body) {
    if (!body || typeof body !== "object" || Array.isArray(body)) {
        return { ok: false, error: "invalid_body" };
    }
    const input = body;
 ...
 */
function validatePatchField(key, value) {
    switch (key) {
        case "submittedName":
