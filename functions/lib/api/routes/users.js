...
const CREATE_USER_RATE_PER_HOUR = rateLimits_1.RATE.CREATE_USER_PER_ADMIN.limit;
const DELETE_USER_RATE_PER_HOUR = rateLimits_1.RATE.DELETE_USER_PER_ADMIN.limit;
const UPDATE_USER_RATE_PER_HOUR = rateLimits_1.RATE.UPDATE_USER_PER_ADMIN.limit;
const SET_PASSWORD_RATE_PER_HOUR = rateLimits_1.RATE.SET_PASSWORD_PER_ADMIN.limit;
const MAX_DISPLAY_NAME_LEN = limits_1.MAX_LEN.NAME;
 ...
 */
exports.usersRouter.get("/groom-profiles", auth_2.requireAuth, async (_req, res) => {
    try {
        const snap = await (0, database_1.getDatabase)().ref("groomProfiles").get();
        const val = snap.val() ?? {};
 ...
 */
exports.usersRouter.put("/groom-profiles/:uid", auth_2.requireAuth, auth_2.requireAdmin, async (req, res) => {
    ...
    try {
        await (0, database_1.getDatabase)().ref(`groomProfiles/${uid}`).set(data);
        res.json({ ok: true });
    }
    catch (err) {
 ...
 */
exports.usersRouter.delete("/groom-profiles/:uid", auth_2.requireAuth, auth_2.requireAdmin, async (req, res) => {
    const { uid } = req.params;
    try {
        await (0, database_1.getDatabase)().ref(`groomProfiles/${uid}`).remove();
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ error: "write_failed", detail: errorMessage(err) });
 ...
 */
exports.usersRouter.get("/", auth_2.requireAuth, auth_2.requireAdmin, async (_req, res) => {
    try {
        const snap = await (0, database_1.getDatabase)().ref("users").get();
        const val = snap.val() ?? {};
        const list = Object.entries(val).map(([uid, data]) => ({
            uid,
            id: uid,
 ...
 *   - audit log entry
 */
exports.usersRouter.post("/", auth_2.requireAuth, auth_2.requireAdmin, (0, rateLimit_1.uidRateLimit)("createUser", CREATE_USER_RATE_PER_HOUR, ONE_HOUR_MS), async (req, res) => {
    ...
    try {
        ...
        await (0, audit_1.writeAudit)(callerUid, "createPortalUser", {
            uid: userRecord.uid,
            role: input.role,
        });
 ...
 * arbitrary user."
 */
exports.usersRouter.get("/:uid", auth_2.requireAuth, async (req, res) => {
    const { uid } = req.params;
 ...
 */
exports.usersRouter.patch("/:uid", auth_2.requireAuth, auth_2.requireAdmin, async (req, res) => {
    const { uid } = req.params;
    ...
    try {
        await (0, database_1.getDatabase)().ref(`users/${uid}`).update(patch);
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ error: "write_failed", detail: errorMessage(err) });
 ...
 */
exports.usersRouter.put("/:uid", auth_2.requireAuth, auth_2.requireAdmin, (0, rateLimit_1.uidRateLimit)("updateUser", UPDATE_USER_RATE_PER_HOUR, ONE_HOUR_MS), async (req, res) => {
    const callerUid = req.caller.uid;
    const { uid } = req.params;
    const input = (req.body ?? {});
    const db = (0, database_1.getDatabase)();
    ...
    const newUsername = input.username !== undefined ? input.username.toLowerCase() : null;
    ...
    try {
        ...
        const usernameChanged = newUsername !== null && newUsername !== profile.username;
        if (roleChanged || usernameChanged) {
            ...
            const { admin: _legacy, ...rest } = existing;
            void _legacy;
 ...
 */
exports.usersRouter.delete("/:uid", auth_2.requireAuth, auth_2.requireAdmin, (0, rateLimit_1.uidRateLimit)("deleteUser", DELETE_USER_RATE_PER_HOUR, ONE_HOUR_MS), async (req, res) => {
    const callerUid = req.caller.uid;
    const { uid } = req.params;
    if (uid === callerUid) {
 ...
 */
exports.usersRouter.put("/:uid/password", auth_2.requireAuth, auth_2.requireAdmin, (0, rateLimit_1.uidRateLimit)("adminSetPassword", SET_PASSWORD_RATE_PER_HOUR, ONE_HOUR_MS), async (req, res) => {
    const callerUid = req.caller.uid;
    const { uid } = req.params;
    ...
    try {
        await (0, auth_1.getAuth)().getUser(uid); // 404s if missing
    }
    catch {
        res.status(404).json({ error: "not_found" });
        return;
 ...
 */
exports.usersRouter.post("/:uid/admin-claim", auth_2.requireAuth, auth_2.requireAdmin, async (req, res) => {
    const callerUid = req.caller.uid;
    const { uid } = req.params;
