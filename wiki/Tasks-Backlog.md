...
- **TASK-TPL-2 (IN PROGRESS 2026-07-15)** — build the **4 webgency-inspired bespoke templates** (`destination-love` → `dolce-vita` → `sacred-garden` → `blossom-oud`). **Phase 0 plumbing DONE** (bespoke metadata + `useIntroPhase` contract + lazy render seam + filtered editor + backend↔shared sync test). Remaining: scout + build each template (bespoke View + sealed-tap Intro + WebGL effects + native palette + curated variants), register (shared/frontend/backend), browser-verify, commit — one shippable template at a time. **Wave 1 usability gate** ([[Usability Templates Test Plan]]) after template 1 gates the rest. See [[Digital Invitations]] "DECIDED + BUILDING". Plan: `~/.claude/plans/go-to-this-website-snuggly-curry.md`.
