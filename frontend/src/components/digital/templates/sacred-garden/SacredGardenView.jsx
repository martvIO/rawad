// The Sacred Garden — top-level view. Same prop contract as DigitalInvitationView
// (so TemplateRenderer forwards to it unchanged) and the same design-doc fields
// (no new schema). It owns the embossed-envelope intro (via the shared
// useIntroPhase contract), the drifting petals, and the torn-paper section bands.
...
import { sgTokens } from "./tokens.js";
import { SgStyles } from "./Styles.jsx";
...
// Deterministic petal placement — no Math.random, so a re-render never
// reshuffles the garden (and two guests see the same calm drift).
const PETALS = [
  { left: 6, delay: 0, dur: 15, size: 9 },
  { left: 22, delay: 4, dur: 19, size: 12 },
  { left: 38, delay: 8, dur: 16, size: 8 },
  { left: 55, delay: 2, dur: 21, size: 11 },
  { left: 71, delay: 11, dur: 17, size: 9 },
  { left: 87, delay: 6, dur: 20, size: 13 },
];

export function SacredGardenView({
  design,
  guestName,
  guestPhone = "",
  lang = "ar",
  ...
  const t = useMemo(() => sgTokens(design?.themeColor), [design?.themeColor]);
  ...
  // No lazy 3D scene here (the garden is CSS/SVG), so the page is ready at mount.
  // Report it or every sacred-garden visit would be missing its load metric.
  ...
  // The blessing leads the hero, the way the source opens on the Bismillah.
  ...
  useEffect(() => {
    ...
    const els = Array.from(root.querySelectorAll(".sg-scroll"));
  ...
  return (
    <div
      ref={rootRef}
      className={"tpl-sg" + (opened ? " is-opened" : "")}
      lang={lang}
      dir="rtl"
    ...
    >
      ...
      {/* Drifting petals — only on the real page, and never under reduced motion. */}
      {isPublic && !reduced && (
        <div className="sg-petals" aria-hidden="true">
          {PETALS.map((p, i) => (
            <span
              key={i}
              className="sg-petal"
