// Blossom & Oud sections. Same design-doc fields as every template (no new
// schema); RSVP + countdown route through the shared hooks. The mihrab arch
// frames the hero — the source's signature, reinterpreted.
import { useRsvpForm } from "../../../../hooks/useRsvpForm.js";
import { useCountdown } from "../../../../hooks/useCountdown.js";
import { PhoneInput } from "../../../PhoneInput.jsx";
import { ArchFrame, Girih, Blossom, ArabesqueBand, SectionTitle, BoButton, L } from "./parts.jsx";

// ── Hero: the invitation inside a mihrab arch ──────────────────────────────
export function Hero({ t, lang, guestName, namesLine, eyebrow, blessing, venueLine, clipTextOk }) {
  return (
    <header style={{ padding: "44px 20px 26px" }}>
      <div className="bo-reveal d1">
        <ArchFrame t={t} minH={340}>
          <div style={{ textAlign: "center" }}>
            ...
            <div className="bo-track" style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", color: t.paperInkSoft }}>
            ...
            <div aria-hidden="true" style={{ display: "flex", justifyContent: "center", margin: "12px 0" }}>
              <ArabesqueBand t={t} w={180} />
            </div>
            {guestName && (
...
export function CountdownSection({ t, lang, weddingDate, dateText, clipTextOk }) {
  ...
  return (
    <section className="bo-scroll" id="bo-countdown" style={{ padding: "30px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 9, maxWidth: 360, margin: "0 auto" }}>
        {tiles.map(([val, label], i) => (
          <div key={i} className="bo-card" style={{ padding: "13px 4px", textAlign: "center" }}>
...
export function StorySection({ t, lang, items, clipTextOk }) {
  return (
    <section className="bo-scroll" id="bo-story" style={{ padding: "30px 20px" }}>
      ...
      <div style={{ maxWidth: t.maxW, margin: "0 auto", display: "grid", gap: 12 }}>
        {items.map((it, i) => (
          <div key={i} className="bo-card" style={{ padding: "16px 18px" }}>
            {it.when && <div className="bo-track" style={{ fontSize: 9.5, fontWeight: 800, textTransform: "uppercase", color: t.paperInkSoft }}><bdi dir="ltr">{it.when}</bdi></div>}
...
export function ScheduleSection({ t, lang, items, clipTextOk }) {
  return (
    <section className="bo-scroll" id="bo-events" style={{ padding: "30px 20px" }}>
          ...
          return (
            <div key={i} style={{ position: "relative", paddingInlineStart: 36, paddingBottom: last ? 0 : 20 }}>
              ...
              <span aria-hidden="true" style={{ position: "absolute", insetInlineStart: 0, top: 0, width: 21, display: "flex", justifyContent: "center" }}>
                {e.icon ? <span style={{ fontSize: 15 }}>{e.icon}</span> : <Blossom size={20} t={t} />}
              </span>
              {e.title && <div style={{ fontWeight: 800, fontSize: 15, color: t.theme.text }}>{e.title}</div>}
              {e.time && <div className="bo-track" style={{ fontSize: 10, fontWeight: 800, color: t.theme.accent, marginTop: 3 }}><bdi dir="ltr">{e.time}</bdi></div>}
...
export function VenueSection({ t, lang, venue, venueCity, venueAddress, accessNote, hotels, clipTextOk }) {
  ...
  return (
    <section className="bo-scroll" id="bo-venue" style={{ padding: "30px 20px" }}>
      <SectionTitle eyebrow={L(lang, "المكان", "המקום")} title={L(lang, "أين نحتفل", "איפה חוגגים")} t={t} clipTextOk={clipTextOk} />
      <div className="bo-card" style={{ maxWidth: t.maxW, margin: "0 auto", textAlign: "center", padding: "22px 18px" }}>
        {venue && <div style={{ fontWeight: 900, fontSize: 19, color: t.paperInk }}>{venue}</div>}
        {venueCity && <div className="bo-track" style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", color: t.paperInkSoft, marginTop: 6 }}>{venueCity}</div>}
      ...
      {hotels.length > 0 && (
        <div style={{ maxWidth: t.maxW, margin: "12px auto 0", display: "grid", gap: 8 }}>
          {hotels.map((h, i) => (
            <div key={i} className="bo-card" style={{ padding: "10px 14px", display: "flex", justifyContent: "space-between", gap: 10 }}>
...
export function DressCodeSection({ t, lang, dressCode, clipTextOk }) {
  return (
    <section className="bo-scroll" id="bo-dress" style={{ padding: "30px 20px" }}>
      ...
      <div className="bo-card" style={{ maxWidth: t.maxW, margin: "0 auto", textAlign: "center", padding: "22px 18px" }}>
        ...
        <div aria-hidden="true" style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 14 }}>
          {[t.blossom, t.trimSoft, t.trim, t.paperSoft].map((c, i) => (
...
export function RsvpSection({ t, lang, opts, mealOptions, guestPhone, onSubmitRsvp, disabled, alreadyAnswered, rsvpDone, clipTextOk }) {
  ...
  return (
    <section className="bo-scroll" id="rsvp" style={{ padding: "30px 20px" }}>
      ...
      <div style={{ maxWidth: 400, margin: "0 auto", position: "relative", background: t.theme.cardBg, border: `1px solid ${t.theme.cardBorder}`, borderRadius: 6, padding: 20 }}>
          ...
          <div data-testid="rsvp-success" role="status" aria-live="polite" style={{ textAlign: "center", padding: "10px 0" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 17, color: t.theme.text }}>
              {alreadyAnswered && !f.showDone
                ...
                : (f.status === "absent" ? L(lang, "نشكر إعلامكم", "תודה שהודעת") : L(lang, "بانتظاركم بكل الحب", "מחכים לכם באהבה"))}
            </div>
          ...
          <>
            ...
            {f.status && (
              <div style={{ marginBottom: 12 }}>
                <label htmlFor="bo-rsvp-phone" style={{ display: "block", fontSize: 12, fontWeight: 700, color: t.theme.accent, marginBottom: 6 }}>{L(lang, "رقم هاتفك", "מספר טלפון")}</label>
                <PhoneInput value={f.phone} onChange={f.setPhone} lang={lang} inputId="bo-rsvp-phone" />
              </div>
            )}
            ...
            {f.status === "attending" && opts.song && (
              <div style={{ marginBottom: 12 }}>
                <label htmlFor="bo-rsvp-song" style={{ display: "block", fontSize: 12, fontWeight: 700, color: t.theme.accent, marginBottom: 6 }}>{L(lang, "أغنية تحبّونها", "שיר שתאהבו")}</label>
                <input id="bo-rsvp-song" value={f.song} onChange={(e) => f.setSong(e.target.value.slice(0, 120))} placeholder={L(lang, "اسم الأغنية...", "שם השיר...")} dir="auto" style={input} />
              </div>
            )}
            <div style={{ marginBottom: 14 }}>
              <label htmlFor="bo-rsvp-note" style={{ display: "block", fontSize: 12, fontWeight: 700, color: t.theme.accent, marginBottom: 6 }}>{L(lang, "رسالة للعروسين", "ברכה לזוג")}</label>
              <textarea id="bo-rsvp-note" rows={3} value={f.note} onChange={(e) => f.setNote(e.target.value.slice(0, 500))} placeholder={L(lang, "مبروك مقدماً...", "מזל טוב מראש...")} style={{ ...input, resize: "vertical" }} />
            </div>
            {f.error && <div role="alert" style={{ color: t.theme.rsvpAbsent, fontSize: 13, marginBottom: 12, textAlign: "center" }}>{f.error}</div>}
            <BoButton t={t} full onClick={f.submit} disabled={f.busy || !f.status || disabled} testid="bo-rsvp-submit">
        ...
        <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
          {f.hearts.map((h) => (
            <span key={h.id} style={{ position: "absolute", bottom: 0, left: `${h.left}%`, color: t.blossom, fontSize: 20, animation: "bo-float 2s ease-out forwards", animationDelay: `${h.delay}s` }}>♥</span>
          ))}
        </div>
        <style>{"@keyframes bo-float{to{transform:translateY(-120px);opacity:0}}"}</style>
      </div>
    </section>
  );
}
...
export function GiftSection({ t, lang, giftNote, giftIban, clipTextOk }) {
  return (
    <section className="bo-scroll" id="bo-gift" style={{ padding: "30px 20px" }}>
...
export function GuestbookSection({ t, lang, wishes, approvedWishes, clipTextOk }) {
  ...
  return (
    <section className="bo-scroll" id="bo-guestbook" style={{ padding: "30px 20px" }}>
      ...
      <div style={{ maxWidth: t.maxW, margin: "0 auto", display: "grid", gap: 10 }}>
        {all.map((w, i) => (
          <div key={i} className="bo-card" style={{ padding: "14px 16px" }}>
