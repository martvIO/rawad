...
// Admin-only: every groom's guests, flattened.
export function subscribeAllGuests(cb) {
  return onValue(ref(db, "guestsByGroom"), (snap) => {
    const out = [];
    snap.forEach((groomBucket) => {
      const groomUid = groomBucket.key;
      groomBucket.forEach((g) => { out.push({ id: g.key, groomUid, ...g.val() }); });
    });
    cb(out);
  });
}
...
export function subscribeGuestsForGroom(groomUid, cb) {
  ...
  return onValue(ref(db, `guestsByGroom/${groomUid}`), (snap) => {
    const out = [];
    snap.forEach((g) => { out.push({ id: g.key, groomUid, ...g.val() }); });
    cb(out);
  });
}
