// Shared building blocks for the digital invitation sections — the section
// header, the floral flourish ornament, the fixed top-bar buttons, and the
// "ink on gold" constant. Everything here is presentational and theme-driven.

export const ON_GOLD = "#2a0f00"; // dark ink for text sitting on the gold gradient

// ── Shared section header ────────────────────────────────────────────────────
export function SectionHead({ eyebrow, title, sub, theme, font }) {
  return (
...
export function FloralFlourish({ theme, width = 230, className = "" }) {
...
export function LangToggle({ lang, setLang, theme, font }) {
  return (
    <div
      style={{
        position: "fixed",
...
export function SorekButton({ lang, isPublic = true, onClick, theme, font }) {
  return (
    <button
      onClick={onClick}
