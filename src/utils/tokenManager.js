...
/**
 * Hydrate the in-memory state from localStorage. Call once at app startup
 * (before any fetch). Safe to call multiple times.
 */
export function loadStoredTokens() {
  try {
    memoryState = {
      idToken: localStorage.getItem(STORAGE_KEY_ID_TOKEN) || null,
      refreshToken: localStorage.getItem(STORAGE_KEY_REFRESH_TOKEN) || null,
      expiresAt: parseInt(localStorage.getItem(STORAGE_KEY_EXPIRES_AT) || "0", 10) || 0,
      uid: localStorage.getItem(STORAGE_KEY_UID) || null,
    };
...
export function storeTokens(tokens) {
  ...
  try {
    localStorage.setItem(STORAGE_KEY_ID_TOKEN, memoryState.idToken);
    localStorage.setItem(STORAGE_KEY_REFRESH_TOKEN, memoryState.refreshToken);
    localStorage.setItem(STORAGE_KEY_EXPIRES_AT, String(memoryState.expiresAt));
    if (memoryState.uid) localStorage.setItem(STORAGE_KEY_UID, memoryState.uid);
  } catch (err) {
